...
| `chips/` | Label imagery chips, 512 m per sample block at both epochs, 4-band: `alaska_vivid_2020_50cm` (1024 px) and `alaska_vivid_2023_30cm` (1707 px) | USDA NRCS ImageServer REST export, apps.geo.fpac.usda.gov; fetched 2026-08-12 by `05.scripts/04a_fetch_chips.R` | **Licensed vendor imagery, not public domain.** Service metadata read 2026-08-12 states the Alaska Vivid imagery is "a licensed dataset from DigitalGlobe with Maxar Intelligence", licensed to USDA-FPAC in 2020, `copyrightText` "Images provided by © 2020 Maxar Technologies Inc." The export endpoint is open; the pixels are copyrighted | 320 chips, both epochs complete over the 160 locked blocks, plus 74 tuning chips over the published delineation areas. Process locally, never redistribute pixels; publish derived crowns and severity fractions only. An earlier entry here recorded "no licence text on either item", which was wrong: the licence text sits in the service metadata, which had not been read. Whether this imagery may be used at all under the project's public-data rule is Seamus's decision. |
